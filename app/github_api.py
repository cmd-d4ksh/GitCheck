...
HEADERS = {
    "Authorization": f"token {TOKEN}",   # GitHub PATs use 'token' not 'Bearer'
...
OSI_LICENSES = {
    ...
    "BSD-2-Clause", "BSD-3-Clause", "MPL-2.0", "EPL-2.0", "AGPL-3.0",
    "ISC", "Unlicense", "CDDL-1.0"
}
...
def file_exists(owner: str, repo: str, path: str):
...
def get_workflow_files(owner: str, repo: str):
    ...
    try:
        ...
        if not isinstance(data, list):
            return []
        return [item.get("name", "").lower() for item in data]
    except Exception:
        return []
...
def get_commit_activity_timeline(owner: str, repo: str, retries: int = 2):
    """Get weekly commit totals for the last 52 weeks."""
    ...
    while attempt <= retries:
        try:
            ...
            if response.status_code == 202:
                # GitHub is generating stats; retry shortly
                if attempt < retries:
            ...
            if not isinstance(data, list):
                return None
            # Each item: { "total": int, "week": unix_ts, "days": [int x7] }
            return [
                {"week": item.get("week"), "total": item.get("total", 0)}
                for item in data
            ]
        except Exception:
            return None
...
def get_repo_metadata(repo_url: str):
    ...
    try:
        ...
        has_dependabot = file_exists(owner, repo, ".github/dependabot.yml") or file_exists(
            owner, repo, ".github/dependabot.yaml"
        )
        has_security_policy = file_exists(owner, repo, "SECURITY.md") or file_exists(
            owner, repo, ".github/SECURITY.md"
        )
        has_code_scanning = any("codeql" in name or "code scanning" in name for name in workflow_files)

        license_info = data.get("license")
        spdx_id = (license_info or {}).get("spdx_id")
        osi_approved = spdx_id in OSI_LICENSES
        ...
        return {
            ...
            "code_scanning": has_code_scanning
        }
